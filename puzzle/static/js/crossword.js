...
function Crossw1rd(container_id) {
  ...
  this.initDimensions = function() {
    for (var i=0; i<this.clues.length; i++) {
      var c = this.clues[i];
      if (c.d=='A' && (c.y + c.a.length)>this.width) {this.width = c.y + c.a.length; console.log(c); console.log(this.width);}
      if (c.d=='D' && (c.x + c.a.length)>this.height) this.height = c.x + c.a.length;
    }
  }
  ...
  this.clue_click = function(ev) {
    ...
    var cell = self.cells[clue.x][clue.y];
    cell.click();
  ...
  this.checkWord = function() {
    ...
    for (var i=0; i<this.clues.length; i++) {
      ...
      if (clue.d==this.direction && clue.n==wordNum) {
        for (var c=0; c<clue.a.length; c++) {
          if (clue.d=='A') {
            this.checkCell(this.cells[clue.x][clue.y+c]);
          } else {
            this.checkCell(this.cells[clue.x+c][clue.y]);
          }
        }
        break;
      }
